...
{
    BIT_FILE *bit_file;
    bit_file =(BIT_FILE*)calloc(1,sizeof(BIT_FILE)); //把bit_file初始化为BIT_FILE型的全为0的动态数组
    if (bit_file == NULL)
        return (bit_file);
...
{
    BIT_FILE *bit_file;
    bit_file =(BIT_FILE*)(1,sizeof(BIT_FILE));
    if (bit_file == NULL)
        return (bit_file);
    bit_file->file = fopen(name, "rb");
    bit_file->rack = 0;
...
void CloaseInputBitFile(BIT_FILE *bit_file)
{
    fclose(bit_file->file);
    free((char *)bit_file);
}
...
void OutputBits(BIT_FILE *bit_file, unsigned long code, int count)
{
    ...
    while (mask != 0)
    {
        ...
        if (bit_file->mask == 0)
        {
            if (putc(bit_file->rack, bit_file->file) != bit_file->mask)
